...
def test_create_contract(db_session, admin_user, client_obj):
    ...
    assert c.commercial_contact_id == client_obj.commercial_contact_id


def test_create_contract_with_selected_commercial(
    db_session, admin_user, client_obj, other_commercial
):
    """Vérifie la sélection explicite du contact commercial à la création."""
    ctrl = ContractController(db_session)
    c = ctrl.create_contract(
        admin_user,
        "CT-SEL",
        client_obj.id,
        9000.0,
        commercial_contact_id=other_commercial["employee_id"],
    )
    assert c.commercial_contact_id == other_commercial["employee_id"]


def test_create_contract_amount_remaining_default(
    db_session, admin_user, client_obj
):
    """Vérifie la valeur par défaut de amount_remaining."""
    ctrl = ContractController(db_session)
    c = ctrl.create_contract(
...
def test_commercial_cannot_create_contract(
    ...
    """Vérifie qu'un commercial ne peut pas créer de contrat."""
    ctrl = ContractController(db_session)
...
def test_create_contract_duplicate_number(
    ...
    """Vérifie qu'un numéro de contrat en doublon est refusé."""
    ctrl = ContractController(db_session)
...
def test_gestion_can_update_any_contract(
    ...
    """Vérifie qu'un gestionnaire peut modifier n'importe quel contrat."""
    ctrl = ContractController(db_session)
...
def test_commercial_can_update_own_contract(
    ...
    """Vérifie qu'un commercial peut modifier son propre contrat."""
    ctrl = ContractController(db_session)
...
def test_commercial_cannot_update_other_contract(
    ...
    """Vérifie qu'un commercial ne peut pas modifier le contrat d'un autre."""
    ctrl = ContractController(db_session)
...
def test_commercial_cannot_sign_contract(
    ...
    """Vérifie qu'un commercial ne peut pas signer un contrat."""
    ctrl = ContractController(db_session)
